...
CREATE TABLE FEC.CANDIDATE_MASTER_FILE (
...
CREATE TABLE FEC.COMMITTEE_MASTER_FILE (
    ...
    CAND_ID              varchar(9) REFERENCES FEC.CANDIDATE_MASTER_FILE(CAND_ID)
);

CREATE TABLE FEC.CONTRIBUTIONS_TO_COMMITTEES_FROM_COMMITTEES (
    CMTE_ID            varchar(9) REFERENCES FEC.COMMITTEE_MASTER_FILE(CMTE_ID),
    AMNDT_IND          varchar(1),
    RPT_TP             varchar (3),
...
CREATE TABLE FEC.OPERATING_EXPENDITURES (
    CMTE_ID          varchar(9) REFERENCES FEC.COMMITTEE_MASTER_FILE(CMTE_ID),
    AMNDT_IND        varchar(1),
    RPT_YR           smallint,
    RPT_TP           varchar(3),
...
CREATE TABLE FEC.CANDIDATE_TO_COMMITTEE_LINKAGE (
    CAND_ID           varchar(9) REFERENCES FEC.CANDIDATE_MASTER_FILE(CAND_ID),
    CAND_ELECTION_YR  smallint,
...
CREATE TABLE FEC.CONTRIBUTIONS_BY_INDIVIDUALS (
    CMTE_ID          varchar(9) REFERENCES FEC.COMMITTEE_MASTER_FILE(CMTE_ID) ,
    AMNDT_IND        varchar(1),
    RPT_TP           varchar(3),
    TRANSACTION_PGI  varchar(5),
